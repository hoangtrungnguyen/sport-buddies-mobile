...
class SupabaseScheduleRepository implements ScheduleRepository {
  ...
  /// Runs [body] under this repository's uniform error envelope: a predictable
  /// [ScheduleRepositoryException] passes straight through (the UI shows its
  /// reason); any other error is logged against `SupabaseScheduleRepository.[op]`
  /// before rethrowing, so it reaches the [Bloc.observer]. Every public method
  /// wraps its body in this — one place to evolve the contract.
  Future<T> _guard<T>(String op, Future<T> Function() body) async {
    try {
      ...
      appLogger.e('SupabaseScheduleRepository.$op', error: e, stackTrace: st);
      rethrow;
    }
  }
  ...
  @override
  Future<List<ScheduleCourt>> getCourts() => _guard('getCourts', () async {
  ...
  Future<List<OccupancyDay>> getMonthOccupancy(
      ...
      _guard('getMonthOccupancy', () async {
        final courts = await _ownerCourts();
  ...
  Future<int> createRecurringSlots(
    ...
    int weeks,
      ...
      _guard('createRecurringSlots', () async {
  ...
  @override
  Future<void> blockTime(BlockTimeRequest req) => _guard('blockTime', () async {
        final date = resolveDate(req.date, req.weekday);
  ...
  void _assertBlockable(
    ...
    if (overlaps.any(
        (r) => r['status'] == kStatusBooked || r['status'] == kStatusPending)) {
      throw ScheduleRepositoryException(
    ...
    for (final r in overlaps) {
      ...
      if (s.isBefore(startAt) || e.isAfter(endAt)) {
        throw ScheduleRepositoryException('Khung giờ chồng một phần slot trống '
  ...
  @override
  Future<Slot> approveSlot(String slotId) => _guard('approveSlot', () async {
        ...
        final refreshed =
            await enrichSlotsFromBookings(_client, [slotFromRow(row)]);
        return refreshed.first;
      });

  @override
  Future<void> rejectSlot(String slotId) => _guard('rejectSlot', () async {
        // READ: resolve the pending bookings row behind the slot.
        final bookingId = await pendingBookingIdForSlot(_client, slotId);
  ...
  @override
  Future<void> cancelSlot(String slotId) => _guard('cancelSlot', () async {
